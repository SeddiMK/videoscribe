...
import { Video, Upload as UploadIcon, Youtube, Settings, ArrowRight } from "lucide-react";
...
const Upload = () => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (droppedFile) {
      ...
      if (validTypes.includes(droppedFile.type)) {
        if (droppedFile.size <= 500 * 1024 * 1024) { // 500 MB
          setFile(droppedFile);
  ...
  const handleStartProcessing = () => {
    ...
    sessionStorage.setItem('transcription-settings', JSON.stringify({
      mode: uploadMode,
      fileName: file?.name,
      youtubeUrl,
      language,
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={model} onValueChange={setModel}>
                    ...
                    <SelectContent>
                      <SelectItem value="tiny">Tiny (быстрая, менее точная)</SelectItem>
                      <SelectItem value="base">Base (рекомендуется)</SelectItem>
                      <SelectItem value="small">Small (точнее, медленнее)</SelectItem>
                      <SelectItem value="medium">Medium (очень точная)</SelectItem>
                      <SelectItem value="large">Large (максимальная точность)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
